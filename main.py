...
def get_args():
    ...
    parser.add_argument('--merge_step', type=int, default=1, help='merge bag number step')
...
class MT_Dataset(Dataset):
    def __init__(self, slide_ids, feat_dir, split, MT=False, merge_num=1, ins_label_dir=None):
        self.slide_ids = slide_ids # wsi_label
        self.feat_dir = feat_dir
        self.split = split
        self.MT = MT
    ...
    def __len__(self):
        return len(self.pse_slide_ids)  #规定划分尾包总个数——wsi总数*最大尾包数
    ...
    def get_labeled_feat(self):
        ...
        for i in np.unique(list(self.slide_ids.values())): # labels
            labeled_feat_files.update({i: []})
        for i in list(self.slide_ids.keys()): # wsi_name
            pseudo_num = len(self.split.get(i)["data"]) if self.split is not None else 1 # 卫宝数量
            for j in range(pseudo_num): # 卫宝数量
    ...
    def __getitem__(self, idx):
        ...
        if self.MT:
            if self.split is not None:
                pse_index = self.split.get(slide_name)["data"][int(pse_name)]  #得到尾包对应的pse下标
...
def model_infer(model_suffix, model, feat, label, criterion, ema=False):
...
def MIL_train_epoch(round_id, epoch, model, optimizer, loader, criterion, device, num_classes, model_suffix='ABMIL',
                    ema_model=None, cst_criterion=None, global_step=0):
    global class_th
    ...
    with tqdm(total=len(loader)) as pbar:
        for _, sample in enumerate(loader):
            ...
            loss, logit = model_infer(model_suffix, model, feat, label, criterion)
            if ema_model is not None:
                ema_loss, ema_logit = model_infer(model_suffix, ema_model, ema_feat, label, criterion)
...
if __name__ == '__main__':
    ...
    if args.pseudo_step >= args.max_pseudo_num:  
        ...
        if args.testmode == 0:
            ...
            split_list = list(range(2, args.max_pseudo_num + 1, args.pseudo_step))
