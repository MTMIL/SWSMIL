...
class MULTI_LAYER_MAMBA_MIL(nn.Module):
    def __init__(self,n_classes,dropout,act):
        super(MULTI_LAYER_MAMBA_MIL, self).__init__()
        self.L = 2048 #512
        self.D = 128 #128
        self.K = 1
        self.feature = [nn.Linear(2048, 512)]
        self.mamba_block1 = Mamba(d_model=1024, d_state=128, d_conv=4, expand=2)
        self.mamba_block2 = Mamba(d_model=2048, d_state=128, d_conv=4, expand=2)
        if act.lower() == 'gelu':
    ...
    def forward(self, x, return_attn=False,no_norm=False):
        ...
        x_reverse_mamba = torch.flip(x_mamba, [1])
        x_reverse_mamba_reverse = torch.flip(x_reverse_mamba, [1])
        x_total = torch.cat([x_mamba,x_reverse_mamba_reverse],dim=2)
        x_total = torch.softmax(x_total, dim=2)
        # print(x_total.shape)
        # 第二层
        
        feature = self.feature(x_total)
        x_total_mamba = self.mamba_block2(feature)
        x_total_reverse = torch.flip(x_total_mamba, [1])
        x_total_reverse_mamba = self.mamba_block2(x_total_reverse)
        x_total_reverse_mamba_reverse = torch.flip(x_total_reverse_mamba, [1])
        x_total_total = torch.cat([x_total_mamba,x_total_reverse_mamba_reverse],dim=2)
        x_total_total = torch.softmax(x_total_total, dim=2)
