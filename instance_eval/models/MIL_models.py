...
class ABMIL(nn.Module):
    ...
    def forward(self, wsi_h, need_feature=False):
...
class Feat_Classifier(nn.Module):
    def __init__(self, n_classes=2):
        ...
        fc_size = 1024
        self.attention_net = nn.Linear(fc_size, 512)
        self.classifiers = nn.Linear(512, n_classes)

    def forward(self, patch_h):
        patch_h = patch_h.squeeze(0)
        dim = self.attention_net(patch_h)
        logits = self.classifiers(dim)
        return logits
...
class ResNetTrunk(ResNet):
    ...
    def forward(self, x, feat_only=True):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
